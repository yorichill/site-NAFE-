...
function SocialFeed({ accent }) {
  ...
  // Pinned tweet logic for SocialFeed
  ...
  // Combine and sort by date, preserving pinned tweet at the top
  const unpinned = [
    ...tweets.filter(t => t.id !== pinnedTweet?.id).map(t => ({ type: 'tweet', date: t.created_at, data: t })),
    ...posts.map(p => ({ type: 'post', date: p.createdAt, data: p })),
    ...news.map(n => ({ type: 'news', date: n.createdAt || n.date, data: n }))
  ].sort((a, b) => new Date(b.date) - new Date(a.date));

  const combined = pinnedTweet 
    ? [{ type: 'tweet', date: pinnedTweet.created_at, data: pinnedTweet, isPinned: true }, ...unpinned]
    : unpinned;

  return (
    <div className="nafe-social-feed">
      {combined.map((item, idx) => {
        if (item.type === 'tweet') return <TweetItem key={idx} tweet={item.data} accent={accent} isPinned={item.isPinned} />;
...
function TweetItem({ tweet, accent, isPinned }) {
  const date = new Date(tweet.created_at).toLocaleDateString("fr-FR", { day: "numeric", month: "short" }).toUpperCase();
  return (
    <div className={`nafe-feed-item nafe-clip-card ${isPinned ? "nafe-hub-tweet-card--pinned" : ""}`}>
      {isPinned ? (
        <div className="nafe-feed-tag" style={{ background: "var(--nafe-water-blue)" }}>ÉPINGLÉ · TWITTER</div>
      ) : (
        <div className="nafe-feed-tag" style={{ background: '#1d9bf0' }}>TWITTER</div>
      )}
      <div className="nafe-feed-item__meta nafe-mono">
        <span style={{ color: isPinned ? "var(--nafe-denim-blue)" : "#1d9bf0" }}>@NAFEOFFICIEL</span>
        <span>· {date}</span>
      </div>
      ...
      <div style={{ marginTop: 16, fontSize: 11, display: "flex", justifyContent: "space-between", alignItems: "center" }} className="nafe-mono">
        <span style={{ opacity: 0.5 }}>{tweet.public_metrics?.like_count || 0} LIKES · {tweet.public_metrics?.retweet_count || 0} RETWEETS</span>
        <a href={`https://x.com/NafeOfficiel/status/${tweet.id}`} target="_blank" rel="noopener noreferrer" style={{ color: "var(--nafe-denim-blue)", textDecoration: "none", fontWeight: 700 }}>
          VOIR SUR TWITTER →
        </a>
      </div>
    </div>
